...
void GBDisassemble::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDX_Control(pDX, IDC_BREAK_ADDR, break_address);
...
BEGIN_MESSAGE_MAP(GBDisassemble, CDialog)
  ...
  ON_WM_VSCROLL()
  //}}AFX_MSG_MAP
  ON_EN_CHANGE(IDC_ADDRESS, &GBDisassemble::OnEnChangeAddress)
  ON_BN_CLICKED(IDC_BREAKPOINT, &GBDisassemble::OnBnClickedBreakpoint)
  ON_BN_CLICKED(IDC_REMOVE_BREAKPOINT, &GBDisassemble::OnBnClickedRemoveBreakpoint)
END_MESSAGE_MAP()
...
void GBDisassemble::OnEnChangeAddress()
{
...
void GBDisassemble::OnBnClickedBreakpoint()
{
	CString buffer;
	break_address.GetWindowText(buffer);
	breaking = true;
	break_addr = strtol(buffer, NULL, 16);

	char s[256];
	sprintf(s, "Added breakpoint: %x", break_addr);
	OutputDebugString(s);
...
void GBDisassemble::OnBnClickedRemoveBreakpoint()
{
	breaking = false;
	emulating = 1;
}
