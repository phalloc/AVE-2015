...
public:
	virtual BOOL InitInstance()
	{
		...
		//dlg->Create(IDD_DISASSEMBLE, CWnd::FromHandle(td->handler));
		CWinThread::InitInstance();
		m_pMainWnd = new Disassemble();
...
UINT thread_func(LPVOID lpParameter)
{
	thread_data *td = (thread_data*)lpParameter;
	...
	if (theApp.cartridgeType == 0) {
		...
		dlg->Create(IDD_GB_DISASSEMBLE, CWnd::FromHandle(td->handler));
		dlg->ShowWindow(SW_SHOW);
	}
...
void MainWnd::OnNewWindow(){
	//MessageBoxThread* pMessageBoxThread = new MessageBoxThread();
	//pMessageBoxThread->CreateThread();

	thread_data * threadparams = new thread_data(AfxGetApp()->m_pMainWnd->m_hWnd);
	//threadparams.handler = AfxGetApp()->m_pMainWnd->m_hWnd;

	CWinThread* pthread1;
	pthread1 = (CWinThread*)AfxBeginThread(thread_func, (LPVOID)threadparams, THREAD_PRIORITY_NORMAL, 0, 0, 0);
	//CreateThread(NULL, 0, thread_func, new thread_data(AfxGetApp()->m_pMainWnd->m_hWnd), 0, 0);
}

void MainWnd::OnUpdateNewWindow(CCmdUI* pCmdUI)
{
...
{
	char s[256];
	sprintf(s, "On Tools Disassemble!");
	OutputDebugString(s);
  if(theApp.cartridgeType == 0) {
    Disassemble *dlg = new Disassemble();
...
{
	char s[256];
	sprintf(s, "On Update Tools Disassemble!");
	OutputDebugString(s);
  pCmdUI->Enable(theApp.videoOption <= VIDEO_6X);
}
